...
class IdealRobot:   
    ...
    def draw(self, ax, elems):         ### call_agent_draw
        elems += ax.plot([e[0] for e in self.poses], [e[1] for e in self.poses], linewidth=1.0, color="black")

        ex = [e[0] + math.cos(e[2])*self.r for e in self.poses]
        ey = [e[1] + math.sin(e[2])*self.r for e in self.poses]

        #elems += ax.plot(ex, ey, linewidth=1.0, color="red")

        x, y, theta = self.pose  
        xn = x + self.r * math.cos(theta)  
        yn = y + self.r * math.sin(theta)  
        c = patches.Circle(xy=(x, y), radius=self.r, fill=True, linewidth=2.0, color="white") 
        elems.append(ax.add_patch(c))
        elems += ax.plot([x,xn], [y,yn], linewidth=2.0, color=self.color)
        c = patches.Circle(xy=(x, y), radius=self.r, fill=False, linewidth=2.0, color=self.color) 
        elems.append(ax.add_patch(c))
