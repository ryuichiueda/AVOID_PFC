...
class HazardMarkMcl:    ###mlparticle（12〜18行目）
    def __init__(self, envmap, init_pose, num, motion_noise_stds={"nn":0.19, "no":0.001, "on":0.13, "oo":0.2},                  distance_dev_rate=0.14, direction_dev=0.05):
        self.particles = [HazardMarkParticle(init_pose, 1.0/num) for i in range(num)]
        self.map = envmap
    ...
    def draw(self, ax, elems):  
        hps = [p for p in self.particles if p.hazard ]
        nhps = [p for p in self.particles if not p.hazard ]

        for ps in [hps, nhps]:
            xs = [p.pose[0] for p in ps]
            ys = [p.pose[1] for p in ps]
            vxs = [math.cos(p.pose[2])*p.weight*len(self.particles) for p in ps] #重みを要素に反映
            vys = [math.sin(p.pose[2])*p.weight*len(self.particles) for p in ps]  #重みを要素に反映

            c = 'red' if ps is hps else 'blue'
            elems.append(ax.quiver(xs, ys, vxs, vys, color=c, angles='xy', scale_units='xy', scale=1.5, alpha=0.5)) #変更
